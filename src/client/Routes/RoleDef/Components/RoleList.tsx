...
class RoleListComponent extends React.Component<RoleListProp, RoleListState> {
  ...
  */
  ...
  private bulkOptions = () => {
    return [{
      content: <Checkbox label={'Show Deleted'} onChange={e => this.toggleCheckBox()} checked={this.state.checkboxValue} />
    }]
  };
  ...
  toggleCheckBox = () => {
    ...
    setTimeout(() => {
      this.props.handleCheckBox(this.state.checkboxValue);
    }, 500);
  }
  handleSearch = (val: any) => {

    this.setState({ searchData: val })
    this.props.searchByNameAndDescription(val)
  }
  ...
  static getDerivedStateFromProps = (nextProps: any, prevState: any) => {
    if (nextProps.roleDefs === undefined) {
      return {
        loadingRole: true
      }
    }
  ...
  render() {
    const { actionInProgress, bulkAction, dropdownEle, filterConfig, hideRow, loadingRole, searchData } = this.state;
    const {
      roleDefs,
      theme,
    } = this.props;
          ...
          <div className={theme.pageContainer}>
            ...
            >
              ...
              <div className={searchFieldStyle}>
                <TextField
                  disabled={this.props.roleDefs !== undefined  ? false : true}
                  label="Find a Role..."
                  suffix={<Icon source="search" componentColor="inkLighter" />}
                  value={this.state.searchData}
                  onChange={(text: any) => { this.handleSearch(text) }}
                />
              </div>
